...
function PlaceList({ searchQuery, checkedCriteria, customCriteria }) {
  ...
  let currentPlaces = places.filter(
    (place) => place.category_id === currentCategoryID && place.group_id === currentGroupID
  );
  ...
  if (customCriteria.length !== 0 && arrayIndex.length !== 0) {
    currentPlaces = currentPlaces.map((place) => ({
      ...place,
      numReviews: reviews.filter((review) => review.place_id === place.place_id).length,
      avgRating: reviews
    ...
    currentPlaces.forEach((place) => {
      place.avgRating = place.avgRating.reduce((p, c) => p + c, 0) / place.avgRating.length;
    });
  } else {
    currentPlaces = currentPlaces.map((place) => ({
      ...place,
      ...
      avgRating:
        reviews
          .filter((review) => review.place_id === place.place_id)
          .map((review) => review.rating.reduce((p, c) => p + c, 0) / review.rating.length)
          .reduce((p, c) => p + c, 0) /
  ...
  currentPlaces = currentPlaces.sort(
    (place1, place2) =>
      (place2.avgRating ? place2.avgRating : 0) - (place1.avgRating ? place1.avgRating : 0)
  );
  ...
  if (searchQuery) {
    ...
    currentPlaces = currentPlaces.filter((place) => re.test(place.address) || re.test(place.name));
  }
